<?php
/**
 * The template for displaying 404 pages (Not Found).
 *
 * @package dilletantish
 */

get_header(); ?>

	<div class="row">
		<div class="small-12 medium-8 columns">

				<header class="page-header">
					<h1 class="page-title"><?php _e( 'Oops! That page can&rsquo;t be found.', 'dilletantish' ); ?></h1>
				</header><!-- .page-header -->
		</div>
		<div class="small-12 medium-4 columns">
			<div class="panel">
				<div class="widget">
					<p><?php _e( 'It looks like nothing was found at this location. Maybe try one of the links below or a search?', 'dilletantish' ); ?></p>

					<?php get_search_form(); ?>

					<?php the_widget( 'WP_Widget_Recent_Posts' ); ?>

					<?php if ( dilletantish_categorized_blog() ) : // Only show the widget if site has multiple categories. ?>
					<div class="widget widget_categories">
						<h2 class="widgettitle"><?php _e( 'Most Used Categories', 'dilletantish' ); ?></h2>
						<ul>
						<?php
							wp_list_categories( array(
								'orderby'    => 'count',
								'order'      => 'DESC',
								'show_count' => 1,
								'title_li'   => '',
								'number'     => 10,
							) );
						?>
						</ul>
					</div><!-- .widget -->
					<?php endif; ?>
					</div>
				</div><!-- .page-content -->

		</div>
	</div>

<?php get_footer(); ?>